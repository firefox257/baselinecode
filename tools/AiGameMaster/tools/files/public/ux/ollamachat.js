...
// The Ollama server URL and endpoints
const OLLAMA_SERVER_URL = "http://192.168.50.110:8888";
const OLLAMA_API_URL = `${OLLAMA_SERVER_URL}/api/`;
const OLLAMA_GENERATE_URL = `${OLLAMA_API_URL}generate`;
const OLLAMA_MODELS_URL = `${OLLAMA_API_URL}tags`;
const OLLAMA_CHAT_URL = `${OLLAMA_API_URL}chat`;

// Local Storage Keys
const LOCAL_STORAGE_MODELS_KEY = 'ollamaChatModels';
const LOCAL_STORAGE_SELECTED_MODEL_KEY = 'ollamaChatSelectedModel';
const LOCAL_STORAGE_SYSTEM_PROMPT_KEY = 'ollamaChatSystemPrompt';
const LOCAL_STORAGE_MESSAGES_KEY = 'ollamaChatMessages';
const LOCAL_STORAGE_SELECTED_SYSTEM_PROMPT_TITLE_KEY = 'ollamaChatSelectedSystemPromptTitle';
const LOCAL_STORAGE_SELECTED_CODE_FILTER_KEY = 'ollamaChatSelectedCodeFilter';

class OllamaChat extends HTMLElement {
    constructor() {
        super();
    ...
    async fetchModels() {
        ...
        try {
            ...
            this.models = data.models.map(m => ({ name: m.name, description: m.name })); // Ollama returns models with a 'name' field
            this.populateModelDropdown();
            ...
            console.error("[OllamaChat] Error fetching models:", error);
    ...
    clearConversation() {
        ...
        this.chatSession = createSession(this.systemPrompt, this.currentModel, OLLAMA_CHAT_URL);
    }

    stopOllamaChatResponse() {
        if (this.currentStreamReader) {
            try {
                this.currentStreamReader.cancel();
            } catch (e) {
                console.error('[OllamaChat] Error cancelling stream reader:', e);
            } finally {
                this.currentStreamReader = null;
            }
        }
                ...
                if (this.onOpenCallback) {
                    try {
                        ...
                        if (chatData && chatData.model && chatData.history) {
                            ...
                            console.error('[OllamaChat] onOpenCallback did not return valid chat data.');
                        }
                    } catch (e) {
                        console.error('[OllamaChat] Error loading conversation:', e);
                    }
                }
            });
        }
    ...
    loadConversation(model, history) {
        ...
        this.chatSession = createSession(this.systemPrompt, this.currentModel, OLLAMA_CHAT_URL, history);
        this.renderHistory();
    }
    ...
    async sendMessage(userPromptFromSTT = null) {
        ...
        // Get the full chat history, including the new user prompt.
        const messages = this.chatSession.getHistory();
        messages.push({ role: 'user', content: userPrompt });

        // Construct the payload as it will be sent to the API.
        const debugPayload = {
            model: this.currentModel,
            messages: messages,
            stream: true,
        ...
        // Use a try/catch block to format and log the JSON
        try {
            console.log("JSON Payload being sent:");
            console.log(JSON.stringify(debugPayload, null, 2));
        } catch (e) {
            console.error("Error converting payload to JSON string:", e);
        }
        // --- END DEBUGGING CODE ---
        ...
        try {
            ...
            console.error("[OllamaChat] Error streaming response:", error);
...
if (!customElements.get('ollama-chat')) {
    customElements.define('ollama-chat', OllamaChat);
}
    ...
    mutations.forEach(mutation => {
        if (mutation.type === 'childList') {
            mutation.addedNodes.forEach(node => {
                if (node.nodeType === 1 && node.tagName === 'OLLAMA-CHAT') {
                }
            });
        }
    });
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('ollama-chat').forEach(chatElement => {
    });
});
