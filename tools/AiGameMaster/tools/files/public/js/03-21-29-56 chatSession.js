...
// Global variables for default role names
let DEFAULT_USER_ROLE = 'user';
let DEFAULT_ASSISTANT_ROLE = 'assistant';

/**
 * Sets the global role names for user and assistant messages.
 * These will be used by all new and existing ChatSession instances.
 * @param {string} userRoleName - The desired name for the user role (e.g., "Human", "Client").
 * @param {string} assistantRoleName - The desired name for the assistant role (e.g., "AI", "Bot", "Server").
 */
function setChatRoleNames(userRoleName, assistantRoleName) {
    if (typeof userRoleName !== 'string' || userRoleName.trim() === '') {
        throw new Error("User role name must be a non-empty string.");
    }
    if (typeof assistantRoleName !== 'string' || assistantRoleName.trim() === '') {
        throw new Error("Assistant role name must be a non-empty string.");
    }
    DEFAULT_USER_ROLE = userRoleName;
    DEFAULT_ASSISTANT_ROLE = assistantRoleName;
    console.log(`[ChatSession] Global role names set: User='${DEFAULT_USER_ROLE}', Assistant='${DEFAULT_ASSISTANT_ROLE}'`);
}
...
class ChatSession {
    ...
    addUserMessage(text) {
        ...
        this.messages.push({ role: DEFAULT_USER_ROLE, content: text }); // Use global user role
        console.log(`[ChatSession] User message added with role '${DEFAULT_USER_ROLE}': "${text.substring(0, 50)}..."`);
    }
    ...
    addAssistantMessage(text) {
        ...
        this.messages.push({ role: DEFAULT_ASSISTANT_ROLE, content: text }); // Use global assistant role
        console.log(`[ChatSession] Assistant message added with role '${DEFAULT_ASSISTANT_ROLE}': "${text.substring(0, 50)}..."`);
    }
...
export { getAvailableModels, createSession, setChatRoleNames };
