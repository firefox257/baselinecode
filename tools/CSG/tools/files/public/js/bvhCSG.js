...
function hashVertex4(v) {
    return `${hashNumber(v.x)},${hashNumber(v.y)},${hashNumber(
        v.z
    )},${hashNumber(v.w)}`
}
...
function hasOverlaps(arr) {
    ...
    for (let i = 0, l = arr.length; i < l - 1; i++) {
        ...
        if (
            info1.start < info0.end &&
            Math.abs(info1.start - info0.end) > 1e-5
        ) {
            return true
        }
    }
...
class RaySet {
    ...
    findClosestRay(ray) {
        ...
        function skipRay(r0, r1) {
            const distOutOfThreshold =
                r0.origin.distanceTo(r1.origin) > DIST_EPSILON
            const angleOutOfThreshold =
                r0.direction.angleTo(r1.direction) > ANGLE_EPSILON
...
class HalfEdgeMap {
    ...
    updateFrom(geometry) {
        ...
        const hashFunction = useAllAttributes
            ? hashAllAttributes
            : hashPositionAttribute
        ...
        if (matchDisjointEdges) {
            const { fragmentMap, disjointConnectivityMap } =
...
class TriangleSplitter {
        ...
        if (Array.isArray(tri)) {
            for (let i = 0, l = tri.length; i < l; i++) {
                ...
                if (i === 0) {
                    ...
                    Math.abs(1.0 - t.getNormal(_vec$1).dot(normal)) > EPSILON
                ) {
                    throw new Error(
    ...
    splitByTriangle(triangle) {
        ...
        if (
            Math.abs(1.0 - Math.abs(_triangleNormal.dot(normal))) <
            PARALLEL_EPSILON
        ) {
    ...
    splitByPlane(plane, clippingTriangle) {
        ...
        for (let i = 0, l = triangles.length; i < l; i++) {
            ...
            for (let t = 0; t < 3; t++) {
                ...
                if (
                    didIntersect &&
                    !(_vec$1.distanceTo(_edge$2.start) < EPSILON)
                ) {
            ...
            if (
                !coplanarEdge &&
                intersects === 2 &&
                _foundEdge.distance() > COPLANAR_EPSILON
            ) {
                if (vertexSplitEnd !== -1) {
...
class TypeBackedArray {
    ...
    setType(type) {
        if (this.length !== 0) {
            throw new Error(
                'TypeBackedArray: Cannot change the type while there is used data in the buffer.'
            )
        }
    ...
    setSize(size) {
        ...
        const bufferType = areSharedArrayBuffersSupported()
            ? SharedArrayBuffer
            : ArrayBuffer
        const newArray = new type(
            new bufferType(ceilToFourByteStride(size * type.BYTES_PER_ELEMENT))
        )
        if (this.array) {
...
function getHitSide(tri, bvh) {
    ...
    const hitBackSide = Boolean(
        hit && _ray$1.direction.dot(hit.face.normal) > 0
    )
...
function getHitSideWithCoplanarCheck(tri, bvh) {
    ...
    for (let i = 0; i < total; i++) {
        ...
        if (minDistance <= OFFSET_EPSILON) {
            return hit.face.normal.dot(_normal$1) > 0
                ? COPLANAR_ALIGNED
                : COPLANAR_OPPOSITE
        }
    ...
    a.geometry.boundsTree.bvhcast(b.geometry.boundsTree, _matrix$2, {
        intersectsTriangles(triangleA, triangleB, ia, ib) {
            if (!isTriDegenerate(triangleA) && !isTriDegenerate(triangleB)) {
                ...
                let intersected = triangleA.intersectsTriangle(
                    triangleB,
                    _edge$1,
                    true
                )
                if (!intersected) {
                    ...
                    if (
                        na.dot(nb) === 1 &&
                        Math.abs(pa.constant - pb.constant) <
                            FLOATING_COPLANAR_EPSILON
                    ) {
                        intersected = true
                    }
                }
                ...
                if (intersected) {
                    ...
                    if (_debugContext) {
                        ...
                        _debugContext.addIntersectingTriangles(
                            ia,
                            triangleA,
                            ib,
                            triangleB
...
function appendAttributeFromTriangle(
    ...
    for (const key in attributeData) {
        ...
        if (key === 'position') {
            ...
            _tri$2.a
                .fromBufferAttribute(attr, i0)
                .applyNormalMatrix(normalMatrix)
            _tri$2.b
                .fromBufferAttribute(attr, i1)
                .applyNormalMatrix(normalMatrix)
            _tri$2.c
                .fromBufferAttribute(attr, i2)
                .applyNormalMatrix(normalMatrix)
...
function getOperationAction(operation, hitSide, invert = false) {
    switch (operation) {
        case ADDITION:
            if (
                hitSide === FRONT_SIDE ||
                (hitSide === COPLANAR_ALIGNED && !invert)
            ) {
                return ADD_TRI
            }
        ...
        case HOLLOW_SUBTRACTION:
            if (
                !invert &&
                (hitSide === FRONT_SIDE || hitSide === COPLANAR_OPPOSITE)
            ) {
                return ADD_TRI
            }
        ...
        case HOLLOW_INTERSECTION:
            if (
                !invert &&
                (hitSide === BACK_SIDE || hitSide === COPLANAR_ALIGNED)
            ) {
                return ADD_TRI
            }
...
function performOperation(
    ...
    const { aIntersections, bIntersections } = collectIntersectingTriangles(
        a,
        b
    )
    ...
    const nonHollow =
        operations.findIndex(
            (op) => op !== HOLLOW_INTERSECTION && op !== HOLLOW_SUBTRACTION
        ) !== -1
...
function performSplitTriangleOperations(
    ...
    for (let i = 0, l = splitIds.length; i < l; i++) {
        const ia = splitIds[i]
        const groupIndex =
            groupOffset === -1 ? 0 : groupIndices[ia] + groupOffset
...
function prepareAttributesData(
    referenceGeometry,
    targetGeometry,
    attributeData,
    relevantAttributes
    ...
    for (let i = 0, l = relevantAttributes.length; i < l; i++) {
        ...
        attributeData.initializeArray(
            key,
            aAttr.array.constructor,
            aAttr.itemSize,
            aAttr.normalized
        )
    }
...
function assignBufferData(geometry, attributeData, groupOrder) {
    ...
    for (const key in referenceAttrSet) {
        ...
        for (
            let i = 0,
                l = Math.min(groupOrder.length, attributeData.groupCount);
            i < l;
            i++
        ) {
    ...
    for (
        let i = 0, l = Math.min(groupOrder.length, attributeData.groupCount);
        i < l;
        i++
    ) {
...
class Evaluator {
    ...
    evaluate(a, b, operations, targetBrushes = new Brush()) {
        ...
        if (targetBrushes.length !== operations.length) {
            throw new Error(
                'Evaluator: operations and target array passed as different sizes.'
            )
        }
        ...
        if (displayDisconnectedEdges) {
            if (halfEdges.unmatchedDisjointEdges) {
                ...
                for (let triIndex = offset; triIndex < triCount; triIndex++) {
                    ...
                    for (let e = 0; e < 3; e++) {
                        const otherTriIndex = halfEdges.getSiblingTriangleIndex(
                            triIndex,
                            e
                        )
                        if (otherTriIndex === -1) {
            ...
            for (let triIndex = offset; triIndex < triCount; triIndex++) {
                ...
                if (halfEdges.disjointConnections) {
                    for (let e = 0; e < 3; e++) {
                        const disjointTriIndices =
                            halfEdges.getDisjointSiblingTriangleIndices(
                                triIndex,
                                e
                            )
                        const disjointEdgeIndices =
                            halfEdges.getDisjointSiblingEdgeIndices(triIndex, e)
