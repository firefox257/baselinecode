...
class ChatSession {
                ...
                while (true) {
                    ...
                    if (eventString.startsWith('data:')) {
                        ...
                        try {
                            ...
                            // Suppress noisy logging for [DONE]
                        }
                    }
                }
    ...
    /**
     * Sends a request to the AI and streams the response text chunk by chunk.
     * The callback function is called for each new chunk of text.
     * The onEnd function is called when the stream is complete.
     * @param {string} userMessage - The user's message for the current turn.
     * @param {function(string): void} onChunk - A callback function that receives each text chunk as it arrives.
     * @param {function(): void} onEnd - A callback function that is called when the stream finishes.
     * @param {number} [temperature=0.7] - The sampling temperature for the AI response (0.0 to 1.0).
     * @returns {Promise<void>} A promise that resolves when the stream has completed.
     * @throws {Error} If the initial request fails.
     */
    async requestStreamText(userMessage, onChunk, onEnd, temperature = 0.7) {
        this.addUserMessage(userMessage); // Add user message to history immediately
        ...
        if (typeof onChunk !== 'function' || typeof onEnd !== 'function') {
            throw new Error("onChunk and onEnd must be valid functions.");
        }
        ...
        try {
                ...
                while (true) {
                    ...
                    if (eventString.startsWith('data:')) {
                        ...
                        try {
                            ...
                            // Suppress noisy logging for [DONE] and other non-JSON data
                        }
                    }
                }
                if (streamBuffer.includes('[DONE]')) {
            ...
            this.addAssistantMessage(fullResponseContent); // Add full AI response to history
            onEnd(); // Call the onEnd function when the stream is finished
